...
class robot:
    ...
    def measurement_prob(self, measurements):
        ...
        for i in range(len(measurements)):
            ...
            try:
              ...
              print 'zero division in robot', self, self.bearing_noise, error_bearing
              print e
              raise e
        return error
    ...
    def sense(self, noise): #do not change the name of this function
        ...
        y = self.y
        theta = self.orientation
        #~ print 'Sense from', (x, y)
        
        for L in landmarks:
          # landmark coordinates are stored in (y, x) form for some reason...
          delta_x = L[1] - x
          delta_y = L[0] - y
          beta = atan2(delta_y, delta_x) % (2 * pi)
          alpha = beta - theta
          # if noise parameter != 0, add gaussian noise to bearing
          if noise:
            alpha += random.gauss(0, self.bearing_noise)
          Z.append(alpha)
          #~ print
          #~ print 'delta x =', delta_x
          #~ print 'delta y =', delta_y
          #~ print '(beta, theta, alpha)', (beta, theta, alpha)
        return Z 
    # copy your code from the previous exercise
...
def generate_ground_truth(motions):
    ...
    for t in range(T):
        print 'motion:', t
        print 'robot: ', myrobot
        print 'bearing noise:', myrobot.bearing_noise 
